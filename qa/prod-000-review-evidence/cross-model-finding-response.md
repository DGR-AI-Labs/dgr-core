...
| B6 — canonical direction / droppable assertion | Remediated within the reviewed bound. `ATK_06_TIMEOUT_OUTCOME` is explicitly authoritative T0 policy and CORE-001 is the T3 conformance mirror. Commit `587585c...` makes deletion, rename, or `#[ignore]` of the named equality test fail the required enumeration guard; the guard unit tests exercise present, missing, and ignored states. It does not prove the assertion body remains unchanged, which stays in source/human review scope. |
